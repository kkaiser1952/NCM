	...
	//		$("#cb1span").removeClass("hidden");
      ...
      if (str == "") {
         ...
         //alert('in an else of showActivities()');
            ...
            $.ajax({
               ...
               success: function(response) {
                   ...
                   if ($('#thenetCallsign').length) { // This just tests if it exists
                    ...
                    if (sortOrder) {
                        var sortColName = sortOrder.split(':')[0];
                        var sortDirection = sortOrder.split(':')[1];
                        console.log('@805 '+sortColName+' '+sortDirection);
                        sorttable.innerSortFunction(sorttable.getColumn(document.getElementById("actLog"),sortColName), sortDirection);
                    } else {
                        $('#actLog th.defaultSortHeader')[0].click(); // emulate click on default sort column
                    }
                }
                sorttable.makeSortable(document.getElementById("actLog"));
            ...
            complete: function() {
                // wait until the #actLog element is populated before accessing its rows
                console.log('@611 '+$('#actLog tr').length);
            } 
        });

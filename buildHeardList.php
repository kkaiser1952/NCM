...
// this is done with a SET GLOBAL group_concat_max_len=2048 this makes the max about 292 callsigns
	...
	$sql = $db_found->prepare("
		SELECT GROUP_CONCAT(callsign ORDER BY logdate ASC SEPARATOR '<br>   ') AS callList
	...
	$sql->execute();
		$result = $sql->fetch();
			echo "NCS heard List for Net #$netID<br>Stations heard in order:<br>$result[callList]";
			
			$vert = "NCS heard List for Net #$netID<br>Stations heard in order: $result[callList2]";
			$Vcombo = "NCS heard List for Net #$netID<br>Stations heard in order: $result[callCntyList]";
			$vcomboDist = "NCS heard List for Net #$netID<br>Stations heard in order: $result[callCntyDistList]";
			
			$alphaList = "NCS heard List for Net #$netID<br>$result[callListAlpha]";
			
			$nameAlpha = "NCS heard List for Net #$netID<br>$result[nameAlpha]";
			
			$lowcase = strtolower("NCS heard List for net #$netID<br>$result[lowcase_callList]");

...
// Create five icons from the above PoiIconClass class and one from ObjIconClass
...
var manInTheMiddle_50 = "images/markers/manInTheMiddle_50.png";
    ...
    var objmiddle = objbounds.getCenter(); // alert(middle); //LatLng(-93.20448, 38.902475)
    var padit  = objbounds.pad(.075);   // add a little bit to the corner bounding box
    ...
    var Ose = padit.getSouthEast();
        ...
        var ob1 = new L.marker(new L.latLng( Osw ),{
            ...
            title:'ob1'}).addTo(map).bindPopup('OB1<br>The Objects SW Corner<br>'+sw).openPopup();
        
        var ob2 = new L.marker(new L.latLng( Onw ),{
           icon: L.icon({iconUrl: greenmarkername , iconSize: [32,36] }),
           title:'ob2'}).addTo(map).bindPopup('OB2<br>The Objects NW Corner<br>'+nw).openPopup();
        
        var ob3 = new L.marker(new L.latLng( One ),{
           icon: L.icon({iconUrl: greenmarkername , iconSize: [32,36] }),
           title:'ob3'}).addTo(map).bindPopup('OB3<br>The Objects NE Corner<br>'+ne).openPopup();
        
        var ob4 = new L.marker(new L.latLng( Ose ),{
           icon: L.icon({iconUrl: greenmarkername , iconSize: [32,36] }),
           title:'ob4'}).addTo(map).bindPopup('OB4<br>The Objects SE Corner<br>'+se).openPopup();
    	
        // Add the temp center marker to the map here in the javascript code allows it to use the current map view,
        // When its earlier in the stack, it centers on our house becaue that is the default map location
        var ob5 = new L.marker(new L.latLng( objmiddle ),{
            contextmenu: true, contextmenuWidth: 140, contextmenuItems: [{ 
            text: 'Click here to add mileage circles', callback: circleKoords}],   
            icon: L.icon({iconUrl: manInTheMiddle_50 , iconSize: [32, 36] }),     
            title:'ob5'}).addTo(map).bindPopup('OB5<br>The Objects Center Marker<br>'+middle).openPopup();
        
        // Definition of the 5 markers above, corners plus middle    
        var CornerList = L.layerGroup([ob1, ob2, ob3, ob4, ob5]);
...
var classList = '<?php echo "$classList  ObjectL, CornerL"; ?>'.split(','); //alert(classList); // EOCL,FireL,HospitalL,RepeaterL,SheriffL,SkyWarnL,
// var classList = '<?php echo "$classList"."ObjectL,"."CornerL"; ?>'.split(',');
   // alert(classList);

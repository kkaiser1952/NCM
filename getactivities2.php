...
if ( $q <> 0 ){
	...
	$sql2 = "select sec_to_time(sum(timeonduty)) as tottime 
		...
		$stmt2 -> execute();
		$tottime = $stmt2->fetchColumn(tottime); //echo "tottime = $tottime";
		
		//$stmt2 -> execute(); // commented out 2017-11-22
		//$netcall = trim($stmt2->fetchColumn(1)); //echo "$netcall<br>"; // TE0ST
		
		$stmt2 -> execute(); // commented out 2017-11-22
		$activity = trim($stmt2->fetchColumn(activity)); 

		$stmt2 -> execute(); // commented out 2017-11-22
		$prebuilt = $stmt2->fetchColumn(pb); 
...
/*   $sql9 = "SELECT orgType
    ...
    $stmt9->execute();
    $orgType    = $stmt9->fetchColumn(orgType); */
  // echo "@80   $orgType<br>";
 */  
	...
	$headerAll = '
		...
		<tr>            	
			...
			<th title="Facility" class="besticky c33" > Facility </th>  
			<th title="onSite" class="besticky c34"> onSite </th>
...
*/
	...
	foreach($db_found->query($g_query) as $row) {
        ...
        //echo "@top:  $newFacility <=> $usedFacility<br>";
  //      echo "@326 orgType= $orgType";
        
        // Facility grouping in NCM display is handled here
        if (TRIM($netcall) == 'KCHEART' AND $newFacility !== $usedFacility AND $row[facility] !== '') {
        ...
        } // end of If facility grouping KCHEART
        
        
        //echo "<br>@bottom:  $newFacility <=> $usedFacility<br>";
